...
int seqA, ackA, seqB, ackB;
float estimatedTime = 1.0, devTime = 0.0;

/* RDT3.0 only */
struct pkt *currentApkt;
struct pkt *currentBpkt;
...
char checkChecksum(struct pkt *packet) {
  int sum;
  sum = packet->seqnum;
  sum += packet->acknum;
  sum += packet->checksum;
...
{
  // sender event 1: handle received from above
  struct pkt *pktptr = malloc(sizeof(struct pkt));

  strcpy(pktptr->payload, message.data);
  /* RDT3.0 only */
  pktptr->seqnum = seqA;
  pktptr->acknum = ackB;
...
{
  // sender event 3: handle ACK
  if (checkChecksum(&packet) && packet.acknum == ackA) {
    stoptimer(A);
...
B_input(packet) struct pkt packet;
{
  ...
  // isCorrupt? or hasNextSeq?
  if (packet.acknum != seqB || packet.checksum)
    return;

  /* hasNextSeq && notCorrupt */
